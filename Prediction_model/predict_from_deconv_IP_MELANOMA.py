...
data_version = 'all_deconvolutions_2022-09-29'
...
datasets = ['Gide', 'Riaz', 'Hugo', 'IP']
deconv = None
#read all datasets into one: deconv
...
# %%
## Load all response data and keep only the data from common samples
response = pd.read_csv(path_interm / "response_melanoma_IP.csv", index_col=0)

common_ids = set(response.index).intersection(deconv.index)
df_all = deconv.loc[common_ids, :]
response = response.loc[df_all.index, :]
response.dataset.value_counts()
# %%
...
# rename variables
if data_version == 'all_deconvolutions_2022-09-29':
    dic_rename = {
        'Epidish': 'EpiDISH',
        'Quantiseq': 'quanTIseq',
...
# %%
path_clin = path_interm / "clinic_melanoma_IP.csv"
clin = pd.read_csv(path_clin, index_col=0)
clin.tail()
# %% 
#join response and clinical
...
# X_train = X[df_all.index.str.contains('IP')]
# y_train = y[y.index.str.contains('IP')]
# X_test = X[~X.index.str.contains('IP')]
# y_test = y[~y.index.str.contains('IP')]
# X_train
# df_all
X
# %%
...
for l1_name, l1_ratios in l1_ratios_list:
    ...
    for condi in conditions:
        ...
        for clin_col in ["SEX", "AAGE", "TMB"]: #for test_name in datasets:
            ...
            X = df_all.loc[valid_ids, var_idx].join(add_clin)
            y = response['BOR - binary'].loc[valid_ids]
            # reverse labels to match "equaling progressive disease" objective
            y = -(y-1)
            # stratify train / test by response
            np.random.seed(0)

            ### The normal random split
            X_train, X_test, y_train, y_test = train_test_split(
                X, y, 
                test_size=0.25, 
                random_state=0, 
...
for l1_name, l1_ratios in l1_ratios_list:
    ...
    for condi in conditions:
        ...
        for test_name in ['IP']: # dataset to use for testing
