---
title: "Figure 3 and figure 3 supplementary material"
...
wrapper_tpurity <- function(deconv_data, true_estimates, estimate, cancer_abreviation){
  ...
  title = paste0(estimate, " vs MethylCBS Cancer")
  y_lab = "MethylCBS Cancer"
  final_plot[["ccle"]] <- scatterplot_truevsestimate(merged_subset, estimate, "EpiDISH_MethylCBS_cancer", title, color,  x_lab, y_lab)
  
  # Scatterplot BPRNACan
  title = paste0(estimate, " vs BPmetCan Cancer")
  y_lab = "BPmetCan Cancer"
  final_plot[["bprnacan"]] <- scatterplot_truevsestimate(merged_subset, estimate, "EpiDISH_BPmetCan_cancer", title, color, x_lab, y_lab)
...
plot_ridgeplot <- function(data){
 data %>% dplyr::select(-c(ParticipantBarcode, Study)) %>% data.matrix() %>% rcorr() -> cor.table
  ...
  for (ct in c("Lymphocytes", "Macrophages", "Neutrophils")){
    ...
    # ridgeplot perse
    cols <- c("H&E" = "red", "CIBERSORT" = "yellow", "EpiDISH" = "blue")
...
wrapper_ridgeplot <- function(deconv_data, true_estimates, cancer_abreviation){
  ...
  deconv_subset_summary <- dplyr::select(deconv_subset, matches("Samples|Lymphocytes|Macrophages|Neutrophils"))
...
################################################################################
wrapper_corrplots <- function(deconv_data, true_estimates, cancer_abreviation){
  # build the data
...
```{r}
tcga_deconv <- read_csv("/mnt/SERVER-CRCT-STORAGE/CRCT21/Private/julien/gemdecan/data/all_deconvolutions/TCGA_EpiDISH_CIBERSORT_BPmetCan_MethylCBS.csv") %>%
  dplyr::select(-Sample) %>%
  dplyr::rename(Samples = ...1) %>%
  rename_with(stringr::str_replace, 
              pattern = "Neutrophiles$", replacement = "Neutrophils", matches("Neutrophiles")) %>%
  mutate_at("Samples", str_remove_all, ".01A*") %>%
  mutate_at("Samples", str_replace_all, "-", ".") %>% 
  group_by(Samples) %>%
  summarise_all(mean)

Aran_all_facs <- read_tsv("/mnt/SERVER-CRCT-STORAGE/CRCT21/Private/Julien/gemdecan/data/facs/Aran_facs.csv") %>%
  mutate_at("Sample_ID", str_remove_all, ".01A*") %>%
  mutate_at("Sample_ID", str_replace_all, "-", ".")

HE_all_TCGA <- read_csv("/mnt/SERVER-CRCT-STORAGE/CRCT21/Private/Julien/gemdecan/data/HE_fractions.csv") %>%
  dplyr::select(-c(...1)) %>%
  mutate_at("ParticipantBarcode", str_remove_all, ".01A*") %>%
  mutate_at("ParticipantBarcode", str_replace_all, "-", ".") %>%
  distinct(ParticipantBarcode, .keep_all= TRUE) # for the READ samples TCGA.AG.5 & TCGA.AG.3
...
```{r}
# Lists of celltypes of each signature ( NO NEED IN THIS CASE)
...
signature_dict <- list()
signature_dict[["BPmetCan"]] <- list(Lymphocytes = c("_B$", "CD4", "CD8", "NK", "Treg", "Lymphocytes"),
                    Macrophages = c("Mono", "M0", "M1", "M2", "Macrophages"),
                    Neutrophils = c("Neutrophiles"))

signature_dict[["MethylCBS"]] <- list(Lymphocytes = c("_B", "CD4", "CD8", "NK", "Treg", "Lymphocytes"),
                                                                         Macrophages = c("Mono", "Macrophages"),
                                                                         Neutrophils = c())
...
ggsave("Fig3_summary_corrplot.pdf", dpi = "print", width = 250, height = 100, units = "mm", scale = 1)
```
...
unique_cancers <- dplyr::filter(HE_all_TCGA, ParticipantBarcode %in% tcga_deconv_ridge$Samples) %>% distinct(Study) %>% deframe()
for (cancer in unique_cancers){
  ...
  pdf_name <- paste("Sup.fig3_", cancer, ".pdf", sep = "")
  ggsave(pdf_name, dpi = "print", width = 210, height = 297, units = "mm", scale = 2.5)
}
system("pdfunite Sup.fig3_*.pdf Sup.fig3.pdf")
```
