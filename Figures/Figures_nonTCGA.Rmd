...
## Corrplot
```{r}
corr <- benchmark_corr(deconv_data = deconv_GSE72056, true_estimates = truth_GSE72056, dict = dict_GSE72056, key = "Samples")
p <- plot_corrplots(store_corrs = corr, key = "Samples", title = "GSE72056")
p
...
deconv_data = deconv_GSE72056
true_estimates = truth_GSE72056
dict = dict_GSE72056
key = "Samples"
title = "GSE72056"
...
for (ct in unique(dplyr::select(true_estimates, -any_of(key)) %>% names())){
  ...
  corr_df <- add_row(store_corrs, order) %>% dplyr::mutate(measure2 = factor(measure2, levels = order$measure2)) %>% dplyr::arrange(factor(measure2, levels = order$measure2))
  write.csv( corr_df, paste0('Spearmans_Correlation_',title,'.csv') )
  corr_df %>% ggplot(aes(measure1, measure2, fill=r, label=round(r_if_sig,2))) +
              geom_tile() +
              labs(x = NULL, y = NULL, fill = "Spearman's\nCorrelation", title= title, subtitle="Only significant correlation coefficients shown (95% I.C.)") +
              scale_fill_gradient2(mid="#FBFEF9",low="#0C6291",high="#A63446", limits=c(-1,1)) +
              geom_text() + theme_classic() + rotate_x_text(angle = 45) + theme(axis.text.x=element_markdown()) + theme(axis.text.y=element_markdown()) +
              scale_x_discrete(expand=c(0,0), labels= function(x) highlight(x, "Average", "red")) +
              scale_y_discrete(expand=c(0,0), labels= function(x) highlight(x, "Average", "red")) 
  # order columns by number of NA (push empty columns to the right)
corr_matrix <- corr_df %>%
    dcast(measure2~measure1, value.var = "r") 
corr_matrix_sorted <- corr_matrix[order(rowSums(is.na(corr_matrix))), ]
corr_matrix_sorted2 <- corr_matrix_sorted[,order(colSums(is.na(corr_matrix_sorted))) ]

melt(data = corr_matrix_sorted2) %>% ggplot(aes(measure2, variable, fill=value, label=round(value,2))) +
            geom_tile() + 
            labs(x = NULL, y = NULL, fill = "Spearman's\nCorrelation", title= title, subtitle="Only significant correlation coefficients shown (95% I.C.)") +
            scale_fill_gradient2(mid="#FBFEF9",low="#0C6291",high="#A63446", limits=c(-1,1), na.value = 'white') +
            geom_text() + theme_classic() + rotate_x_text(angle = 45) + theme(axis.text.x=element_markdown()) + theme(axis.text.y=element_markdown()) +
            scale_x_discrete(expand=c(0,0), labels= function(x) highlight(x, "Average", "red")) +
            scale_y_discrete(expand=c(0,0), labels= function(x) highlight(x, "Average", "red")) 
...
corr <- benchmark_corr(deconv_data = deconv_Thomas, true_estimates = truth_Thomas, dict = dict_GSE72056, key = "Samples")
p <- plot_corrplots(store_corrs = corr, key = "Samples", title = "Thomas")
p
