...
################################################################################
benchmark_corrplots <- function(deconv_data, true_estimates, key, title){
  # build the data
  ...
  deconv_subset_summary <- ridgeplot_parse(deconv_data = deconv_subset, signatures_dictionary = signature_dict, key = key)
...
## Data loading
```{r}
deconv <- read_tsv("/mnt/SERVER-CRCT-STORAGE/CRCT21/Private/Miguel/Projects/CRCT/21 - Epigenomics and network modelling of heterogeneity in immuno-oncology/Ting Xie/2021/CIBERSORTx/final_example_signatures/all_deconvolutions/all_deconvolutions_Melanoma_GSE72056_no_malighant.txt") %>% dplyr::rename_with(~str_replace(.,"__", "_CBSX_"))  %>% dplyr::rename_with(~str_replace(.,"__", "_")) #! fix naming of deconv results
...
# signature_dict[["CCLE_TIL10"]] <- list(B = c(),
#                                      CAFs = c(),
#                                      CD4 = c(),
#                                      CD8 = c(),
#                                      Endothelial = c(),
...
```{r}
benchmark_corrplots(deconv_data = deconv, true_estimates = ground_truth,  key = "Samples", title = "GSE72056")
