...
wrapper_ridgeplot <- function(deconv_data, true_estimates, cancer_abreviation){
  ...
  true_subset %>% bind_cols(deconv_subset_summary) %>% select(-c(Samples)) -> data_toplot
...
wrapper_tpurity <- function(deconv_data, true_estimates, cancer_abreviation){
  ...
  final_plot[["ccle"]] <- scatterplot_truevsestimate(merged_subset, "ESTIMATE", "Epidish_CCLE_TIL10_Cancer.cells", title, color)
  
  # Scatterplot BPRNACan
  title = "ESTIMATE vs BPRNACan Cancer"
  final_plot[["bprnacan"]] <- scatterplot_truevsestimate(merged_subset, "ESTIMATE", "Epidish_BPRNACan_cancer", title, color)
...
## A) Tumor Purity
```{r}
wrapper_tpurity(deconv_data = tcga_deconv,true_estimates = Aran_all_facs, cancer_abreviation = "LUAD")
```
...
for (cancer in unique(HE_all_TCGA$Study)){
  ...
  tpurity_by_cancer[[cancer]] <- tryCatch({ wrapper_tpurity(deconv_data = tcga_deconv, true_estimates = Aran_all_facs, cancer_abreviation = cancer) # sometimes there arent enough samples of the tumour type
        },
        error = function(cond) {
...
ridges <- ggarrange(plotlist = ridgeplots_by_cancer[["LUAD"]], ncol = 3, common.legend = T, legend = "bottom")
scater <- ggarrange(plotlist = tpurity_by_cancer[["LUAD"]], ncol = 3, common.legend = T)
blank  <- ggplot() + theme_void()
fullplot <- ggarrange(plotlist = list(scater, ridges, blank), nrow = 3, heights = c(0.15, 0.3, 0.45))
annotate_figure(fullplot, top = text_grob("LUAD", color = "black", size = 30))
pdf_name <- paste("test", "ridges", ".pdf", sep = "")
ggsave(pdf_name, dpi = "print", width = 210, height = 297, units = "mm", scale = 3)
# for (ct in c("Lymphocytes", "Macrophages", "Neutrophils")){
#    annotate_figure(print(ridgeplots_by_cancer[[cancer]][[ct]]),
#                              top = text_grob("Test", color = "black", size = 30),
