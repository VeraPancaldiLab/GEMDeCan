...
for sample_number in $(find $indir/*.fastq.gz | awk -F'/' '{print $NF}'| cut -d'-' -f3 | cut -d'_' -f1 | sort  -u ); do
  echo $sample_number
  BASE_FORMAT="$(find $indir/*.fastq.gz | awk -F'/' '{print $NF}'| cut -d'-' -f 1-2 | sort -u)-${sample_number}" 
  LINE_FORMAT="L00\d"
