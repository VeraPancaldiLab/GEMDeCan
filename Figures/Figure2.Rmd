...
wrapper_corrplots <- function(deconv_data, true_estimates, cancer_abreviation){
  ...
  ct_corrplots <- list()
   for (ct in c("Lymphocytes", "Macrophages", "Neutrophils")){
    ...
    order <- store_corrs %>% group_by(measure2) %>% dplyr::summarize(meanr = mean(r)) %>% arrange(desc(meanr))
    ct_corrplots[[ct]] <- dplyr::mutate(store_corrs, measure2 = factor(measure2, levels = order$measure2)) %>% dplyr::arrange( factor(measure2, levels = order$measure2)) %>%
...
```{r}
wrapper_corrplots(deconv_data = tcga_deconv_ridge, true_estimates = HE_all_TCGA)
```
